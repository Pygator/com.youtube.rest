 ...
 *
 */
public class Pgsql308tube {

	private static DataSource Pgsql308tube = null; //hold the database object
	...
	private static DataSource Pgsql308tubeConn() throws Exception {
		...
		try {
			 ...
			 * Pgsql308tube will hold the database object
			 */
			if (context == null) {
				context = new InitialContext();
			}
			
			Pgsql308tube = (DataSource) context.lookup("jdbc/StudentServDB");
	...
	protected static Connection oraclePcPartsConnection() {
		...
		try {
			conn = Pgsql308tubeConn().getConnection();
			return conn;
		} 
		catch (Exception e) {
			e.printStackTrace();
